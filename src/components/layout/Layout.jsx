...
import {
  ...
  BookOpen, Target, FileText, Settings, Sun, Moon, Menu, Bell, LogOut
} from 'lucide-react'
    ...
    <NavLink to={to} end={to === '/'} style={{ textDecoration: 'none' }} onClick={onClick}>
      {({ isActive }) => (
        ...
        >
          ...
          <span style={{ fontFamily: 'DM Sans, sans-serif', fontWeight: isActive ? 700 : 500, fontSize: 15 }}>{label}</span>
        </div>
      )}
    </NavLink>
  )
    ...
    <>
      ...
      <div style={{ padding: '4px 4px 28px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1.1, fontFamily: 'DM Sans, sans-serif' }}>Kingdom Seekers</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2, fontFamily: 'DM Sans, sans-serif', fontWeight: 500 }}>Ministry Platform</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ paddingTop: 16, borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px' }}>
          ...
          <div>
            <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', fontFamily: 'DM Sans, sans-serif' }}>Theophilus Laryea</p>
            <p style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'DM Sans, sans-serif' }}>System Admin</p>
          </div>
        </div>
        ...
        <button
          onClick={toggle}
          style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 12px', borderRadius: 10, color: '#3d3d4e', background: 'transparent', border: 'none', cursor: 'pointer', width: '100%', fontSize: 15, fontWeight: 500, fontFamily: 'DM Sans, sans-serif', transition: 'background 0.15s' }}
  ...
  return (
    <div style={{ display: 'flex', height: '100svh', overflow: 'hidden', background: 'var(--bg-base)' }}>
      ...
      {mobileOpen && (
        <div
          className="md:hidden"
          style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', background: 'rgba(0,0,0,0.45)' }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0 }}>
        ...
        <div
          className="md:hidden"
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid var(--border)', background: 'var(--bg-surface)', flexShrink: 0 }}
        >
          ...
          <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', fontFamily: 'DM Sans, sans-serif' }}>Kingdom Seekers</span>
